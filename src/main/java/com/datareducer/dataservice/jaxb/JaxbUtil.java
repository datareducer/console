...
import com.datareducer.dataservice.jaxb.atom.Content;
import com.datareducer.dataservice.jaxb.atom.Entry;
import com.datareducer.dataservice.jaxb.atom.Feed;
import com.datareducer.dataservice.jaxb.csdl.*;
import com.datareducer.dataservice.jaxb.register.Element;
...
public class JaxbUtil {
    ...
    private static Pattern recalculationPattern = Pattern.compile("^CalculationRegister_([^_]+)_(?!Base|RowType|ScheduleData|ActualActionPeriod)([^_]+)$");
    // Шаблон имени виртуальной таблицы базовых данных регистра расчета
    private static Pattern baseCalculationRegisterPattern = Pattern.compile("^CalculationRegister_([^_]+)_Base([^_]+)$");
        ...
        for (DataServiceEntity entity : entities) {
            ...
            if (ownerName.startsWith(entity.getResourceName())) {
                ...
                if (entity instanceof AccumulationRegister) {
                    ...
                    for (CalculationRegisterBaseRegister baseRegister : ((CalculationRegister) entity).getBaseRegisters()) {
                        virtualTables.add(baseRegister);
                    }
                    if (!virtualTables.isEmpty()) {
                        setFieldReference(virtualTables, name, ownerName, reference);
                    }
