...
public final class RelationalExpression implements FilterElement, Cloneable {
    ...
    public String getHttpForm() {
        ...
        if (field.isComposite()) {
            ...
            builder.append("datetime'").append(DATE_TIME_FORMATTER.format((Instant) value)).append('\'');
    ...
    public String getSqlForm() {
        StringBuilder builder = new StringBuilder();
        builder.append(field.getName()).append(operator.getSqlForm());
        if (field.isComposite()) {
            String expr;
            try {
                expr = parseCompositeFieldValue((String) value).get("expr");
            } catch (ParseException e) {
                throw new ClientRuntimeException(e);
            }
            builder.append('\'').append(expr).append('\'');
        } else if (value instanceof Instant) {
            builder.append('\'').append(DATE_TIME_FORMATTER.format((Instant) value)).append('\'');
        } else if (value instanceof UUID) {
            builder.append('\'').append(value).append('\'');
        } else if (value instanceof String) {
            builder.append('\'').append(value).append('\'');
