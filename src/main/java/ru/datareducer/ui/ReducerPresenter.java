...
public class ReducerPresenter implements ModelReplacedListener {
    ...
    public void acceptModel(ReducerConfiguration model) {
        ...
        reducer.getModel().getInfoBases().addListener((ListChangeListener<InfoBase>) change -> {
            while (change.next()) {
    ...
    private void registerWindowsPaneEventHandlers() {
        ...
        dataServiceResourceWindowsManager.windowsProperty()
                .addListener((MapChangeListener<DataServiceResource, DataServiceResourceWindow>) change -> {
                    ...
                    if (window != null) {
                        if (change.wasAdded()) {
                            navPane.addWindowIcon(window);
                        } else if (change.wasRemoved()) {
                            navPane.removeWindowIcon(window);
                        }
                    }
                });
    }
